...
def get_j_kpts(mydf, dm_kpts, hermi=1, kpts=np.zeros((1,3)), kpts_band=None, KSCED=0):
    ...
    if hermi == 1 or is_zero(kpts):
        ...
        if KSCED == 1:
            rhoR_b = np.zeros((4,ngrids))
            for ao_ks_etc, p0, p1 in mydf.aoR_loop(mydf.grids, kpts,1):
                ao_ks, mask = ao_ks_etc[0], ao_ks_etc[2]
                for i in range(nset):
                    rhoR_b[:,p0:p1] += make_rho(i, ao_ks, mask, 'GGA').real
                ao = ao_ks = None
            np.save('rhoR_b', rhoR_b)
